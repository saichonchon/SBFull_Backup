trigger MBA_Staging_Account_Trigger on MBA_Staging_Account__c (before insert, before update, after insert, after update) 
{
 ...
 if (Trigger.isBefore)
  ...
  if (Trigger.isAfter)
  {
		...
		string soqlWHERE_OriginalExternalAccountIds = '';
	    ...
	    for (MBA_Staging_Account__c mba : trigger.new)
	    {
	        ...
	        // Grab all the Original Account Ids
	        if (mba.Original_External_Account_ID__c != null)
	        {
	        	if (stMBAAcctId.add(mba.Original_External_Account_ID__c))
	        	{
	        		soqlWHERE_OriginalExternalAccountIds += (soqlWHERE_OriginalExternalAccountIds.length() == 0 ? '' : ', ') + '\'' + mba.Original_External_Account_ID__c + '\'';
	        	}
	        }
	    }
		...
		if (soqlWHERE_OriginalExternalAccountIds != '')
		{
			soql_Sub += ' or MBAAccountID__c IN (' + soqlWHERE_OriginalExternalAccountIds + ')';
		}
	    ...
	    for (MBA_Staging_Account__c mba : trigger.new)
	    {
	        ...
	        // Check if a Account record exists already, either under the mba account id or as the original external id
	        ...
	        else if (mba.Original_External_Account_ID__c != null && mpAcc.containsKey(mba.Original_External_Account_ID__c))
	        {
	        	// Add to the map of the staging record to the existing Account based on the original account id
	        	mpStagingToExisting.put(mba, mpAcc.get(mba.Original_External_Account_ID__c));
